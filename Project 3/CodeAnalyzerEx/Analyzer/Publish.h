...
private:
	string directoryname = "../HtmFolder";
...
{
	ofstream mystyle;
	mystyle.open("../HtmFolder/mystylesheet.css");
...
{
	ofstream myfile;
	myfile.open("../HtmFolder/result.html");
	myfile << "<html>";	
	...
	for (auto i = result.begin(); i != result.end(); i++)//iterating over vector<string> containing file path/names
	{
		...
		string link = "";
		link.append("../HtmFolder/");
		link.append(filenamewohtm);
		link.append(".htm");			
		myfile << "<head>";		
		myfile << "<link rel=" << "stylesheet" << " type=" << "text/css" << " href=" << "../HtmFolder/mystylesheet.css" << ">";
	...
	myfile << "</html>";
	myfile.close();
	string temppath = "../HtmFolder/result.html";
...
void PublishFiles::generatehtmlfile(string pathof){
	...
	file << "<html>" << "<head>";
	file << "<link rel=" << "stylesheet" << " type=" << "text/css" << " href=" << "../HtmFolder/mystylesheet.css" << ">";
	file << "<h3>" << "<b>" << "Dependent Files" << "</b>" << "</h3>";	file << "<hr>";
	file << "<script type=\"text/javascript\" src=\"../CodeAnalyzerEx/HideUnhide.js\"></script>" << std::endl << "</head>";
	...
	while (inputFile.get(next)){
		if (next == '<')
			...
			file << "<div id =" << id << ">";id++;} //Adding unique div id to each button
		else if (next == '}')
