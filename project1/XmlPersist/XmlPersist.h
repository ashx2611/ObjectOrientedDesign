...
//Class contains functions to persist to and from xml
...
template<typename Data>
string XmlPersist<Data>::toxml(NoSqlDb<Data> & db)
{
...
template<typename Data>
Element<StrData> XmlPersist<Data>::fromxml(const string& xml)
{
	try {
		Element<Data> elem;
...
public:	
	string xmlstruct();
};// the Function to demonstrate xml file containing project structure
std::string ProjStruct:: xmlstruct()
{
